...
- [API reference](#api-reference)
  ...
  - [Row fill](#row-fill)
  - [Sequential sweep (`for...of`)](#sequential-sweep-forof)
...
- **`new LiteBinaryReader(source, options)`** -- the reader. `source` is an `ArrayBuffer` or any view (`TypedArray` or `DataView`); `options` is the `{ schema, stride?, count?, littleEndian?, byteOffset? }` layout. The constructor validates and compacts the schema into SoA tables, resolves the owned buffer, and freezes the shape. Build it once per `(source, layout)`; reuse it across the read loop.
  ...
  - **`rows(out)` and `for (const row of reader)`** -- `readRow` as a zero-allocation `for...of` sweep (S12): a hand-written iterator (never a generator) that reuses one sink and one result record, so the whole sweep is `0 B/op` per row. The yielded row is borrowed -- copy what you keep.
...
new LiteBinaryReader(
  ...
  options: {
    schema: { name: string | number; type: TypeCode; offset: number;
              lengthField?: string | number; littleEndian?: boolean }[];
    stride?: number;
    count?: number;
    littleEndian?: boolean;   // reader default; a field's own littleEndian overrides it
    byteOffset?: number;
  }
): LiteBinaryReader
```
...
- **`schema`** -- a non-empty array of `{ name, type, offset }` (with optional `lengthField` and `littleEndian`). `name` is a string or number; `type` is a `T_*` code (integer 0..9); `offset` is a non-negative integer. A duplicate name throws `R_DUPLICATE_FIELD` (a silent shadow is refused); a bad type throws `R_BAD_TYPE`; a bad offset throws `R_BAD_OFFSET`; a non-boolean per-field `littleEndian` throws `R_BAD_SCHEMA`.
...
- **`littleEndian`** -- the reader's default read byte order. Default `true` (the sane wire default). Set `false` for a big-endian buffer. A schema field's own `littleEndian` (S10) overrides it for that field, so one reader reads a MIXED-endian record (e.g. a big-endian length/type prefix in front of a little-endian payload); a field without it inherits this flag.
...
### Sequential sweep (`for...of`)

```ts
rows<T extends { length: number; [i: number]: number | bigint }>(out: T): IterableIterator<T>
[Symbol.iterator](): IterableIterator<RowTuple<S>>
```

`readRow` as a **`for...of`** sweep. `rows(sink)` fills a **caller-owned** sink each row and yields it; a bare `for (const row of reader)` yields a **reader-owned** sink. Both are **zero-allocation per row**:

```js
const sink = new Array(reader.fieldCount);    // allocated once, reused every row
for (const row of reader.rows(sink)) handle(row);   // row === sink, filled in place

for (const row of reader) handle(row);        // reader owns the sink; same guarantee
```

The iterator is **hand-written, not a generator** -- a generator allocates a fresh `{ value, done }` on every `yield`, which the zero-GC gate rejects; this reuses one result record and one sink, so the whole sweep is `0 B/op` per row (its own torture gate holds it there). The cost is one iterator object (and, for the bare form, one owned array) **per sweep** -- cold, resolved once like `field()`.

The yielded object is **borrowed and reused** -- the *same* sink is handed back every row, so read what you need before the next step. `[...reader]` gives you N references to the **one** reused array (every element reads as the last row), by design; to materialize the sweep, copy each row:

```js
const allRows = Array.from(reader, row => row.slice());   // distinct rows
```

Two concurrent sweeps get independent cursors; the bare form allocates a fresh owned sink per call so they never clobber each other. A too-short or `null` sink into `rows` hits the same `R_BAD_LENGTH` door as `readRow` (on the first step). With a `const`-typed schema the yielded value is the S11 `RowTuple<S>` (a per-field `number` / `bigint` tuple); a plain `Field[]` schema keeps the permissive union.
...
A field is lane-**eligible** only when a typed-array lane reads the exact same bytes `getX` would: the **field** is host-endian (its own `littleEndian === IS_LITTLE_ENDIAN`), the field's first byte is naturally aligned (`(base + offset) % width === 0`), and the stride keeps every row aligned (`stride % width === 0`). The endianness test is **per field** (S10), so a mixed-endian reader can hand back a lane on its host-endian fields and decline on its opposite-endian ones. A width-1 field (`T_U8`/`T_I8`) is always eligible on a host-endian field; an unaligned field, an odd stride, or an opposite-endian field **declines** -- `laneOf` returns `null` for it, never a lane that would read wrong bytes. An out-of-range `fieldId` returns `null` too.
...
### Mixed-endian records

`littleEndian` is the reader's default byte order; a schema field may carry its **own** `littleEndian` (S10) to override it for that field. So one reader reads a mixed-endian record -- the common wire shape of a big-endian length/type prefix in front of a little-endian payload:

```js
const r = new LiteBinaryReader(buf, {
  schema: [
    { name: 'len', type: T_U32, offset: 0, littleEndian: false },  // big-endian prefix
    { name: 'val', type: T_F32, offset: 4, littleEndian: true },   // little-endian payload
  ],
  stride: 8,
});
r.getU32(0, r.field('len'));  // read big-endian
r.getF32(0, r.field('val'));  // read little-endian -- same row, same reader
```
...
| `R_BAD_TYPE`         | a `field.type` is not an integer in 0..9 (or an unsupported LBK1 lane_kind) |
...
- **Endianness is owned per field, not just per reader (S10).** `littleEndian` is a reader default; a field may override it, so one reader reads a mixed-endian record. The default is a single `_leOf` byte-order table read on the hot path (one L1 index), chosen over a second getter class for the same reason the benchmark closed the endianness split-class idea -- a predicted index is not worth doubling the getter surface. A field without an override inherits the reader flag, so any pre-S10 schema reads byte-identically (proven by a regression pin and the unchanged 0-B/op gates); `laneOf` declines a field whose endianness differs from the host. No new error code, no type-table move.
...
**126 deterministic tests, all pass**, plus a torture gate that proves leak-freedom (now including a schema-space fuzzer) and a controls run that proves the door.

```bash
npm test                 # 126 node:test cases (contract + boundary + drift guard + cooperation proof + streaming adapter + iterator + hardening gates)
